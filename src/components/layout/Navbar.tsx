...
import { Menu, X, LayoutDashboard, LogOut, GraduationCap, Users } from "lucide-react"
        ...
        <div className="flex items-center gap-4">
          ...
          {isAuthenticated && user ? (
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-72 p-2 shadow-2xl border-border/50 rounded-xl">
                <DropdownMenuLabel className="font-normal p-0 mb-2">
                  <div className="flex items-center gap-4 p-3 rounded-lg bg-muted/40">
                    ...
                    <div className="flex flex-col gap-0.5 overflow-hidden">
                      ...
                      <span className={`mt-1 inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full w-fit ${
                ...
                <div className="p-1">
                  <DropdownMenuItem asChild>
                    <Link to={dashboardLink} className="cursor-pointer py-2.5 px-3 flex items-center rounded-md font-medium text-muted-foreground focus:bg-muted focus:text-foreground">
            ...
            <div className="hidden items-center gap-3 md:flex">
              <Link to="/login">
                <Button variant="ghost" className="font-bold hover:bg-muted/50 rounded-full px-5">
                  Log in
                </Button>
              </Link>
              <Link to="/signup">
        ...
        <div className="md:hidden absolute top-full left-0 w-full bg-background/95 backdrop-blur-xl border-b border-border shadow-2xl animate-in fade-in slide-in-from-top-2">
          <div className="container-padding py-6 space-y-6">
            ...
            <div className="flex justify-between items-center border-b border-border/50 pb-4">
               <span className="text-sm font-bold text-muted-foreground uppercase tracking-widest">Navigation</span>
               <ThemeToggle />
            </div>

            {(isAuthenticated ? authenticatedNavLinks : publicNavLinks).length > 0 && (
            ...
            {isAuthenticated && user ? (
              <div className="bg-muted/30 rounded-xl p-4 border border-border/50 shadow-inner">
                <div className="flex items-center gap-4 mb-4 pb-4 border-b border-border/50">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mb-1 mt-1">{user.email}</p>
              ...
              <div className="flex flex-col gap-3 pt-2">
                <Link to="/login">
                  <Button variant="outline" className="w-full h-12 text-base font-bold rounded-xl" onClick={() => setIsMobileMenuOpen(false)}>
                    Log in
                  </Button>
                </Link>
                <Link to="/signup">
                  <Button className="w-full h-12 text-base font-bold shadow-lg shadow-primary/20 rounded-xl" onClick={() => setIsMobileMenuOpen(false)}>
                    Get Started
                  </Button>
                </Link>
              </div>
